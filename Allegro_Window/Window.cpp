...
ALLEGRO_DISPLAY* Window::Display = nullptr;
ALLEGRO_FONT* Window::Fonte = nullptr;
ALLEGRO_TIMER* Window::timer = nullptr;
...
{
    Size = size;
    MousePosition = glm::ivec2(size / 2);
...
void Window::ProcessEvents()
{
    ...
    case ALLEGRO_EVENT_DISPLAY_RESIZE:
        ...
        Size = glm::ivec2(event.display.width, event.display.height);
        break;
    ...
    case ALLEGRO_EVENT_MOUSE_AXES:
        MousePosition = glm::ivec2(event.mouse.x, event.mouse.y);
...
// set window details
//---------------------------------------------------------------------------------
void Window::SetIconWindow(const std::string& iconPath)
{
    ALLEGRO_BITMAP* icon = al_load_bitmap(iconPath.c_str());
    Error::Test_Entity(icon, "icon Window");
    al_set_display_icon(Display, icon);
}
void Window::SetCursorWindow(const std::string& cursorPath)
{
