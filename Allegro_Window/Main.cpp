...
int main()
{
    Window* window = new Window(ivec2(1600, 900), "Allegro Game");
    ...
    while (window->Run())
    {
        window->SetTitle(string("Allegro5 " + std::to_string(Timer::Frames)));
        ...
        if (window->EventQueueEmpty())
        {
            ...
            al_draw_textf(window->Fonte, Colors::AliceBlue, 10, 10, 0, "Time: %f FPS: %2.f ElapsedTime: %f", Timer::Time, Timer::Frames, Timer::ElapsedTime);
